...
<div id="app" v-on:click="handleAppClick" v-on:dragenter="handleDragEnter" v-on:drop="handleDragDrop">
  ...
  <div  v-on:dragover.prevent v-on:drop="handleDragDrop" v-on:dragleave.self="handleDragLeave" v-show="uploadOverLay.visible" class="upload-overlay">
    <div style="margin: 0px; cursor: pointer;">
      <i v-on:click="uploadOverLay.visible = false" class="material-icons md-48 md-light">close</i>
    </div>
    <div style="position:relative; margin:100px auto; width: 400px;padding: 20px; color: white; text-align:center; font-size:24px;">
  ...
  <div class="contextual-menu" v-bind:style="{ top: contextualMenu.y + 'px', left: contextualMenu.x + 'px' }"v-show="contextualMenu.visible">
    <div v-show="contextualMenuItemVisibility.preview"><i class="material-icons md-18">remove_red_eye</i> Preview</div>
    <div v-show="contextualMenuItemVisibility.copy" v-on:click="handleCopyContextualMenu"><i class="material-icons md-18">content_copy</i> Create copy</div>
    <div v-show="contextualMenuItemVisibility.rename" v-on:click="handleRenameContextualMenu"><i class="material-icons md-18">edit</i> Rename</div>
    <div v-show="contextualMenuItemVisibility.download" v-on:click="handleDownloadContextualMenu"><i class="material-icons md-18">file_download</i> Download</div>
    <div style="border-top: 2px solid #E5E9F2" v-show="contextualMenuItemVisibility.delete" v-on:click="handleDeleteContextualMenu"><i class="material-icons md-18">delete</i> Delete</div>
  </div>

  <!-- Rename Dialog -->
  <el-dialog title="Rename" v-model="renameDialog.visible">
    <el-input v-model="renameDialog.newPath" v-on:keyup.enter.native="handleRenameDialogConfirm" name="renameDialogNewPath"></el-input>
  ...
  <el-dialog title="Create copy" v-model="copyDialog.visible">
    <el-input v-model="copyDialog.newName" v-on:keyup.enter.native="handleCopyDialogConfirm" name="copyDialogNewPath"></el-input>
    <span slot="footer" class="dialog-footer">
  ...
  <el-dialog title="Create new folder" v-model="newFolderDialog.visible">
    <el-input placeholder="Name your new folder" v-model="newFolderDialog.newName" v-on:keyup.enter.native="hanldeNewFolderDialogConfirm" name="newFolderDialogName"></el-input>
    <span slot="footer" class="dialog-footer">
  ...
  <el-row class="header-bar">
    ...
    <!-- <el-col :span="14">
      <el-input icon="search" placeholder="Search for files"> </el-input>
    </el-col>
    <el-col :span="5">&nbsp;</el-col>
    <el-col :span="1">
      <i class="material-icons md-36 md-light">settings</i>
    </el-col> -->
  </el-row>
        ...
        <el-col :span="14">
          ...
          <div style="float:left" class="breadcrumb-view">
            <span v-for="(segment, index) in pathSegments">
              <span v-on:click="handleSegmentClick(segment, $event)" class="segment">{{segment.name}}</span>
              <i v-if="index < pathSegments.length-1" class="material-icons">chevron_right</i>
            </span>
          </div>
          <el-dropdown menu-align="start" size="large">
            <el-button style=" margin-top: 8px; margin-left:16px;"  type="text" >New <i style="vertical-align: text-top" class="material-icons md-18">arrow_drop_down</i></el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-on:click.native="handleNewFolderMenu"><i style="vertical-align:text-bottom; margin-right:6px;" class="material-icons md-24">create_new_folder</i> New folder</el-dropdown-item>
              <el-dropdown-item v-on:click.native="uploadOverLay.visible = true"><i style="vertical-align:text-bottom; margin-right:6px;" class="material-icons md-24">file_upload</i> Upload</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          ...
          <tr >
            <th v-on:click="toggleSort('name', $event)">
                ...
                <span v-if="currentSort.column == 'name'">
                  <i v-if="currentSort.order == 'descending'" class="material-icons md-18">arrow_drop_down</i>
...
<style media="screen">
  * {
  ...
  .material-icons.md-dark { color: rgba(0, 0, 0, 0.3); }
...
export default {
  ...
  computed: {
    ...
    pathSegments () {
      ...
      segments.push({"name": "My drive", "component": "", "path": fullPath});
      for (var i = 0; i < path_components.length; i++) {
        fullPath += "/" + path_components[i];
        var name = path_components[i];
        segments.push({"name": name, "component": path_components[i], "path": fullPath});
      }
      return segments;
    },
  ...
  methods: {
    ...
    handleDeleteContextualMenu(event) {
      ...
      let payload = {"command": "rm", "paths": paths};
      let nodes_to_delete_names = nodes_to_delete.map((r) => r.name);

      $.post("/api", {"payload": JSON.stringify(payload)}, function(data){
    ...
    renameNode(node, newPath) {
      var payload = {"command": "mv", "paths": [node.path], "destination": newPath}
    ...
    copyNode(node, newPath) {
      var payload = {"command": "cp", "paths": [node.path], "destination": newPath}
      var newPathComponents = newPath.split("/");
    ...
    openFolder(path) {
      var payload = {"command": "ls", "path": path};
      $.post("/api", {"payload": JSON.stringify(payload)}, function(data){
    ...
    newFolder(path) {
      var payload = {"command": "mkdir", "path": path};
      this.callApi(payload, function(data) {
        var node = JSON.parse(data);
